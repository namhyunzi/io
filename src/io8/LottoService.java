...
public class LottoService {
	 ...
	 * 1. 회차번호를 획득한다 - 오늘 날짜
	 * 2. 주문번호를 획득한다 - 유닉스타임
	 * 3. 구매금액만큼 로또번호 세트를 자동으로 생성한다.
	 * 4. Order 객체를 생성한다/
	 * 5. Order 객체의 회차번호, 주문번호, 로또번호 세트 리스트를 저장한다.
	...
	/**
	 * 로또번호세트를  자동으로 생성해서 반환한다.
	 * 1. 로또번호를 저장할 TreeSet 객체를 생성한다.
	 ...
	 * 2. 당첨로또번호세트를 획득한다.
	 * 3. 보너스 번호를 획득한다.
	 * 4. 로또 당첨번호를 저장하는 Lotto객체를 생성한다.
	 * 5. Lotto객체에 회차번호, 로또번호세트, 보너스 번호를 저장한다.
	 * 6. Repository객체에 Lotto 객체를 전달해서 저장시킨다.
	 * 7. Lotto객체를 반환한다.
	 * @return 로또 당첨번호 정보
	 */
	public Lotto lotto() {
	 ...
	 * 4. Winning 객체의 당첨여부를 기본적으로 true 로 설정한다.
	 * 5. check(로또번호, 구매한 로또번호세트)를 실행해서 맞는번호세트를 조회한다.
	 * 6. 맞은 번호 개수를 획득한다.
	 * 7. 맞은 번호 개수를 기준으로 등수를 결정한다.
	 * 8. 당첨정보를 저장하는 Winning 객체에 구매한 로또번호세트, 맞은개수, 맞은 번호세트, 등수를 저장한다.
	...
	private List<Winning> check(Lotto lotto, List<Set<Integer>> numbers) {
		...
		for (Set<Integer> set : numbers) {
			...
			// 당첨여부를 true 로 설정하기, 나중에 맞은 갯수가 3개 미만인 경우 false 로 설정할 예정
			winning.setWin(true);
	...
	private Set<Integer> check(Set<Integer> lotto, Set<Integer> numbers) {
		...
		// 구매한 로또번호를 하나씩 꺼내서, 추첨번호에 그 번호가 포함되어있는지 여부를 확인하고
